...
const getClasses = function (responses) {
  ...
  for (j=0; j<mediaQueries.length; j++) {
    if (mediaQueries[j] != '') { css = css + mediaQueries[j] + "{"; } /// Change to omit if nothing for that query
    ...
    for (i=0; i<bootstrap.length; i++) {
      ...
      if(bsTagsAndClasses.indexOf(selector)>-1 && media == mediaQueries[j]) {
